...
Miracle::DirectXTextureCubeArray::DirectXTextureCubeArray(ID3D11Device* a_Device, TextureCubeArray* a_Texture)
...
Miracle::DirectXTextureCubeArray::~DirectXTextureCubeArray()
{
}

std::shared_ptr<Miracle::RenderResource> Miracle::DirectXTextureCubeArray::Create(void* a_Device, GraphicsObject* a_GraphicsResource)
{
	if (a_GraphicsResource->GetType() == GraphicsObject::TEXTURE_CUBE_ARRAY)
	{
		return std::make_shared<DirectXTextureCubeArray>(reinterpret_cast<ID3D11Device*>(a_Device), static_cast<TextureCubeArray*>(a_GraphicsResource));
	}
...
void Miracle::DirectXTextureCubeArray::CreateStaging(ID3D11Device* a_Device, const D3D11_TEXTURE2D_DESC& tx)
{
	D3D11_TEXTURE2D_DESC desc;
	desc.Width = tx.Width;
...
void Miracle::DirectXTextureCubeArray::CreateShaderResourceView(ID3D11Device* a_Device, const D3D11_TEXTURE2D_DESC& tx)
{
...
void Miracle::DirectXTextureCubeArray::CreateUnorderedAccessView(ID3D11Device* a_Device, const D3D11_TEXTURE2D_DESC& tx)
{
